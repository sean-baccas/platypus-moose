...
# now mfem is built, we can do the same for axom too
AXOM_SRC_DIR="$(get_realpath "${SCRIPT_DIR}"/../framework/contrib/axom-repo)"
cd "${SCRIPT_DIR}/.."
git submodule update --init --checkout framework/contrib/axom-repo
...
cd "${SCRIPT_DIR}/.."
git submodule update --init --checkout framework/contrib/tribol-repo
cd $TRIBOL_SRC_DIR
git submodule update --init --recursive
