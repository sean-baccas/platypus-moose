...
/**
 * MFEMCoupledMesh directly constructs an MFEM mesh from a parent libMesh mesh 
 */
class MFEMCoupledMesh : public MFEMMesh
{
public:
  static InputParameters validParams();

  MFEMCoupledMesh(const InputParameters & parameters);

  virtual ~MFEMCoupledMesh();
  ...
  /**
   * Override method in MFEMMesh.
   */
  inline int getLocalMFEMNodeId(const int libmesh_global_node_id) const
  {
    return _mfem_local_node_id_for_libmesh_global_node_id.at(libmesh_global_node_id);
  }

  inline int getLibmeshGlobalNodeId(const int mfem_local_node_id) const
  {
...
protected:
   ...
   */
  void convertSerialDofMappingsToParallel(const libMeshMFEMMesh & serial_mesh,
...
private:
   ...
   */
  std::shared_ptr<libMeshMFEMMesh> _libmesh_mfem_mesh{nullptr};
  /**
