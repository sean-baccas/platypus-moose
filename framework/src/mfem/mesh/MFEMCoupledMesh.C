...
#include "MFEMCoupledMesh.h"

registerMooseObject("MooseApp", MFEMCoupledMesh);

InputParameters
MFEMCoupledMesh::validParams()
{
  InputParameters params = MFEMMesh::validParams();
  return params;
}

MFEMCoupledMesh::MFEMCoupledMesh(const InputParameters & parameters)
  : MFEMMesh(parameters)
{
}

MFEMCoupledMesh::~MFEMCoupledMesh() {}

std::unique_ptr<MooseMesh>
MFEMCoupledMesh::safeClone() const
{
  return std::make_unique<MFEMCoupledMesh>(*this);
}

void
MFEMCoupledMesh::buildBoundaryInfo(std::map<int, std::vector<int>> & element_ids_for_boundary_id,
...
std::vector<int>
MFEMCoupledMesh::getSideBoundaryIDs() const
{
...
bool
MFEMCoupledMesh::isDistributedMesh() const
{
  return (!getMesh().is_replicated() && n_processors() > 1);
}

void
MFEMCoupledMesh::buildCubitBlockInfo(const std::vector<int> & unique_block_ids)
{
  _block_info.setDimension(getMesh().mesh_dimension());
  /**
...
std::vector<int>
MFEMCoupledMesh::getLibmeshBlockIDs() const
{
...
void
MFEMCoupledMesh::buildElementAndNodeIDs(const std::vector<int> & unique_block_ids,
...
void
MFEMCoupledMesh::buildUniqueCornerNodeIDs(
...
std::map<int, int>
MFEMCoupledMesh::getBlockIDForElementID(
...
MFEMCoupledMesh::getBlockIDsForBoundaryID(
    const std::map<int, std::vector<int>> & element_ids_for_block_id,
...
std::unique_ptr<int[]>
MFEMCoupledMesh::getMeshPartitioning()
{
...
{  
  ...
  _mfem_par_mesh =
      std::make_shared<mfem::ParMesh>(MPI_COMM_WORLD, *_libmesh_mfem_mesh, partitioning_raw_ptr);
  ...
  convertSerialDofMappingsToParallel(*_libmesh_mfem_mesh.get(), *_mfem_par_mesh.get());

  _libmesh_mfem_mesh.reset(); // Lower reference count of serial mesh since no longer needed.
...
void
MFEMCoupledMesh::convertSerialDofMappingsToParallel(const libMeshMFEMMesh & serial_mesh,
...
void
MFEMCoupledMesh::buildBoundaryNodeIDs(
